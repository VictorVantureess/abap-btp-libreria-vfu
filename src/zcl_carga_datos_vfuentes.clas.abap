CLASS zcl_carga_datos_vfuentes DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_carga_datos_vfuentes IMPLEMENTATION.

 METHOD if_oo_adt_classrun~main.
    " Definición de tablas internas con el tipo de tus tablas del diccionario
    DATA: lt_acc_categ TYPE TABLE OF ztb_acc_categ__v,
          lt_catego    TYPE TABLE OF ztb_catego_vfuen,
          lt_clientes  TYPE TABLE OF ztb_clientes_vfu,
          lt_clnts_lib TYPE TABLE OF ztb_clnts_lib_vf,
          lt_libros    TYPE TABLE OF ztb_libros_vfuen.

    " ******** TABLA 1: ACCESO CATEGORIAS ********
    lt_acc_categ = VALUE #(
    ...
    DELETE FROM ztb_acc_categ__v.
    INSERT ztb_acc_categ__v FROM TABLE @lt_acc_categ.
    ...
    DELETE FROM ztb_catego_vfuen.
    INSERT ztb_catego_vfuen FROM TABLE @lt_catego.
    ...
    DELETE FROM ztb_clientes_vfu.
    INSERT ztb_clientes_vfu FROM TABLE @lt_clientes.

    IF sy-subrc EQ 0.
      out->write( |Clientes: { sy-dbcnt } registros insertados| ).
    ENDIF.

    " ******** TABLA 4: CLIENTES - LIBROS ********
    lt_clnts_lib = VALUE #(
    ...
    DELETE FROM ztb_clnts_lib_vf.
    INSERT ztb_clnts_lib_vf FROM TABLE @lt_clnts_lib.
    ...
    DELETE FROM ztb_libros_vfuen.
    INSERT ztb_libros_vfuen FROM TABLE @lt_libros.
